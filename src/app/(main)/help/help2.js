import React from "react";
import { MonitorPlay, ListVideo } from "lucide-react";

function Help2() {
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="rounded-[24px] bg-gradient-to-r from-[#062b78] via-[#0b5fe8] to-[#1a9cff] p-6 text-white shadow-[0_18px_45px_rgba(37,99,235,0.22)]">
        <p className="mb-2 inline-flex rounded-full bg-white/15 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.18em] backdrop-blur">
          КаТВ үйлчилгээ
        </p>

        <h2 className="flex items-center gap-3 text-2xl font-black tracking-[-0.4px] md:text-3xl">
          <MonitorPlay  size={28} />
          КаТВ сувгийн жагсаалт
        </h2>

        <p className="mt-3 max-w-[680px] text-sm leading-6 text-white/85 md:text-base">
          Премиум болон стандарт багцын сувгийн дараалал, нэр, харьяа улс, төрөл
          жанрын мэдээлэл.
        </p>
      </div>

      {/* Image */}
      {/* <div className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-[0_14px_35px_rgba(15,23,42,0.08)]">
        <img
          src="/assets/help/katvchannels.jpg"
          alt="КаТВ сувгийн жагсаалт"
          className="h-auto w-full object-cover"
        />
      </div> */}

      {/* Table wrapper */}
      <div className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-[0_18px_45px_rgba(15,23,42,0.08)]">
        <div className="mb-4 flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-blue-600">
            <ListVideo size={21} />
          </div>

          <div>
            <h3 className="text-lg font-black text-[#061f57]">
              Сувгийн дэлгэрэнгүй жагсаалт
            </h3>
            <p className="text-sm text-slate-500">
              Хүснэгтийг баруун, зүүн тийш гүйлгэж үзнэ үү.
            </p>
          </div>
        </div>

        <div
          className="
            max-h-[650px] overflow-auto rounded-2xl border border-slate-200

            [&_table]:w-full
            [&_table]:min-w-[760px]
            [&_table]:border-collapse
            [&_table]:bg-white

            [&_td]:border
            [&_td]:border-slate-200
            [&_td]:px-4
            [&_td]:py-3
            [&_td]:text-sm
            [&_td]:text-slate-700
            [&_td]:align-middle

            [&_tr:first-child_td]:bg-[#062b78]
            [&_tr:first-child_td]:text-white
            [&_tr:first-child_td]:font-bold

            [&_tr:nth-child(even)]:bg-slate-50/70
            hover:[&_tr]:bg-blue-50/60

            [&_p]:m-0
            [&_strong]:font-black
          "
        >
          {/* ЭНД хуучин table-ээ бүгдийг нь үлдээ */}
          <table width="1213">
            <tbody>
              <tr>
                <td>
                  <p>
                    <strong>Премиум багц</strong>
                  </p>
                  <table width="685">
                    <tbody>
                      <tr>
                        <td width="96">
                          <p>
                            <strong>Сувгийн дараалал</strong>
                          </p>
                        </td>
                        <td width="135">
                          <p>
                            <strong>Телевизийн нэр&nbsp;</strong>
                          </p>
                        </td>
                        <td width="120">
                          <p>
                            <strong>Харъяа улс</strong>
                          </p>
                        </td>
                        <td width="279">
                          <p>
                            <strong>Төрөл, жанр</strong>
                          </p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>1</p>
                        </td>
                        <td width="135">
                          <p>National Промо</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>2</p>
                        </td>
                        <td width="135">
                          <p>МҮОНТ&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Олон нийтийн&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>3</p>
                        </td>
                        <td width="135">
                          <p>Монгол ТВ телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>4</p>
                        </td>
                        <td width="135">
                          <p>25-р суваг</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>5</p>
                        </td>
                        <td width="135">
                          <p>ТВ 5 телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>6</p>
                        </td>
                        <td width="135">
                          <p>UBS Телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>7</p>
                        </td>
                        <td width="135">
                          <p>Ийгл ньюс телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>8</p>
                        </td>
                        <td width="135">
                          <p>NTV телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>9</p>
                        </td>
                        <td width="135">
                          <p>ТВ9 телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>10</p>
                        </td>
                        <td width="135">
                          <p>Боловсрол суваг&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>11</p>
                        </td>
                        <td width="135">
                          <p>МNB ММ</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Олон нийтийн&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>12</p>
                        </td>
                        <td width="135">
                          <p>Стар телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>13</p>
                        </td>
                        <td width="135">
                          <p>С1 телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>14</p>
                        </td>
                        <td width="135">
                          <p>SBN телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>15</p>
                        </td>
                        <td width="135">
                          <p>Эх орон телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>16</p>
                        </td>
                        <td width="135">
                          <p>Ви Ти Ви телевиз&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>17</p>
                        </td>
                        <td width="135">
                          <p>ТМ телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>18</p>
                        </td>
                        <td width="135">
                          <p>TV 8 телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>19</p>
                        </td>
                        <td width="135">
                          <p>ETV телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>20</p>
                        </td>
                        <td width="135">
                          <p>Сүлд телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>21</p>
                        </td>
                        <td width="135">
                          <p>Малчин телевиз</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Телевизийн өргөн нэвтрүүлэг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>22</p>
                        </td>
                        <td width="135">
                          <p>MNB world</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Олон нийтийн&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>23</p>
                        </td>
                        <td width="135">
                          <p>mnb sport</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Олон нийтийн&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>24</p>
                        </td>
                        <td width="135">
                          <p>MNB гэр бүл</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Олон нийтийн&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>25</p>
                        </td>
                        <td width="135">
                          <p>ТВ6</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Байгаль, экологи</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>26</p>
                        </td>
                        <td width="135">
                          <p>Mon Plus&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Цэнгээнт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>27</p>
                        </td>
                        <td width="135">
                          <p>Блүүмберг суваг&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм, эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>28</p>
                        </td>
                        <td width="135">
                          <p>MNC&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Мэдээ, мэдээлэл</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>29</p>
                        </td>
                        <td width="135">
                          <p>Өлзий телевиз&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм, эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>30</p>
                        </td>
                        <td width="135">
                          <p>Royal</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм, эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>31</p>
                        </td>
                        <td width="135">
                          <p>Like суваг&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Эрүүл мэнд</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>32</p>
                        </td>
                        <td width="135">
                          <p>Соён гэгээрүүлэгч</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Шинжлэх ухаан, танин мэдэхүй</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>33</p>
                        </td>
                        <td width="135">
                          <p>Eco Channel</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Байгаль, экологи</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>34</p>
                        </td>
                        <td width="135">
                          <p>Seven</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Цэнгээнт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>35</p>
                        </td>
                        <td width="135">
                          <p>О TV&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Цэнгээнт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>36</p>
                        </td>
                        <td width="135">
                          <p>TV 2&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Цэнгээнт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>37</p>
                        </td>
                        <td width="135">
                          <p>Channel11&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Цэнгээнт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>38</p>
                        </td>
                        <td width="135">
                          <p>TV9-2 CINEMA</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Кино суваг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>39</p>
                        </td>
                        <td width="135">
                          <p>Moviemax&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Кино суваг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>40</p>
                        </td>
                        <td width="135">
                          <p>DREAMBOX&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Хүүхэд, боловсрол</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>41</p>
                        </td>
                        <td width="135">
                          <p>MNBA&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Спорт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>42</p>
                        </td>
                        <td width="135">
                          <p>Малчин-Бирж</p>
                        </td>
                        <td width="120">
                          <p>Монгол</p>
                        </td>
                        <td width="279">
                          <p>Танин мэдэхүй</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>43</p>
                        </td>
                        <td width="135">
                          <p>Талын Монгол&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Танин мэдэхүй</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>44</p>
                        </td>
                        <td width="135">
                          <p>GTV&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Эрүүл мэнд</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>45</p>
                        </td>
                        <td width="135">
                          <p>&nbsp;ONTV</p>
                        </td>
                        <td width="120">
                          <p>Нийгэм эдийн засаг</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>46</p>
                        </td>
                        <td width="135">
                          <p>Volume Plus&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Цэнгээнт</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>47</p>
                        </td>
                        <td width="135">
                          <p>E-box</p>
                        </td>
                        <td width="120">
                          <p>Хөгжим</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>48</p>
                        </td>
                        <td width="135">
                          <p>Documentary Box</p>
                        </td>
                        <td width="120">
                          <p>Баримтат кино</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>49</p>
                        </td>
                        <td width="135">
                          <p>Movie box&nbsp;&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Кино суваг</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>50</p>
                        </td>
                        <td width="135">
                          <p>Asian box&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Кино суваг</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>51</p>
                        </td>
                        <td width="135">
                          <p>PSN 1</p>
                        </td>
                        <td width="120">
                          <p>Спорт</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>52</p>
                        </td>
                        <td width="135">
                          <p>PSN 2</p>
                        </td>
                        <td width="120">
                          <p>Спорт</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>53</p>
                        </td>
                        <td width="135">
                          <p>PSN 3</p>
                        </td>
                        <td width="120">
                          <p>Спорт</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>54</p>
                        </td>
                        <td width="135">
                          <p>PSN 4</p>
                        </td>
                        <td width="120">
                          <p>Спорт</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>55</p>
                        </td>
                        <td width="135">
                          <p>PSN 5</p>
                        </td>
                        <td width="120">
                          <p>Спорт</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>56</p>
                        </td>
                        <td width="135">
                          <p>Zvezda</p>
                        </td>
                        <td width="120">
                          <p>Орос</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм, эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>57</p>
                        </td>
                        <td width="135">
                          <p>DW</p>
                        </td>
                        <td width="120">
                          <p>Герман</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм, эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>58</p>
                        </td>
                        <td width="135">
                          <p>France-24</p>
                        </td>
                        <td width="120">
                          <p>Франц</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм, эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>59</p>
                        </td>
                        <td width="135">
                          <p>NHK</p>
                        </td>
                        <td width="120">
                          <p>Япон</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм, эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>60</p>
                        </td>
                        <td width="135">
                          <p>Arirang</p>
                        </td>
                        <td width="120">
                          <p>Солонгос</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм, эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>61</p>
                        </td>
                        <td width="135">
                          <p>Өвөрмонгол</p>
                        </td>
                        <td width="120">
                          <p>Хятад</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм, эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>62</p>
                        </td>
                        <td width="135">
                          <p>NTV rus</p>
                        </td>
                        <td width="120">
                          <p>Орос</p>
                        </td>
                        <td width="279">
                          <p>Цэнгээнт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>63</p>
                        </td>
                        <td width="135">
                          <p>MTV Hits</p>
                        </td>
                        <td width="120">
                          <p>Их Британи</p>
                        </td>
                        <td width="279">
                          <p>Цэнгээнт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>64</p>
                        </td>
                        <td width="135">
                          <p>TV-1000</p>
                        </td>
                        <td width="120">
                          <p>Их Британи</p>
                        </td>
                        <td width="279">
                          <p>Кино суваг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>65</p>
                        </td>
                        <td width="135">
                          <p>Disney</p>
                        </td>
                        <td width="120">
                          <p>АНУ</p>
                        </td>
                        <td width="279">
                          <p>Хүүхэд, боловсрол</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>66</p>
                        </td>
                        <td width="135">
                          <p>2x2&nbsp;&nbsp;&nbsp;&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>Орос</p>
                        </td>
                        <td width="279">
                          <p>Хүүхэд, боловсрол</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>67</p>
                        </td>
                        <td width="135">
                          <p>THT</p>
                        </td>
                        <td width="120">
                          <p>ОХУ</p>
                        </td>
                        <td width="279">
                          <p>Цэнгээнт</p>
                        </td>
                      </tr>
                      {/* <tr>
<td width="96">
<p>68</p>
</td>
<td width="135">
<p>ОРТ1</p>
</td>
<td width="120">
<p>ОХУ</p>
</td>
<td width="279">
<p>Нийгэм эдийн засаг</p>
</td>
</tr> */}
                      <tr>
                        <td width="96">
                          <p>69</p>
                        </td>
                        <td width="135">
                          <p>ROSSIA 1</p>
                        </td>
                        <td width="120">
                          <p>ОХУ</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>70</p>
                        </td>
                        <td width="135">
                          <p>REN&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>ОХУ</p>
                        </td>
                        <td width="279">
                          <p>Нийгэм эдийн засаг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>71</p>
                        </td>
                        <td width="135">
                          <p>Discovery Chennel</p>
                        </td>
                        <td width="120">
                          <p>сингапур</p>
                        </td>
                        <td width="279">
                          <p>Байгаль экологи</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>72</p>
                        </td>
                        <td width="135">
                          <p>Animal Planet</p>
                        </td>
                        <td width="120">
                          <p>ану</p>
                        </td>
                        <td width="279">
                          <p>Байгаль экологи</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>73</p>
                        </td>
                        <td width="135">
                          <p>Galaxy</p>
                        </td>
                        <td width="120">
                          <p>ru</p>
                        </td>
                        <td width="279">
                          <p>Байгаль экологи</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>74</p>
                        </td>
                        <td width="135">
                          <p>Ocean Tv</p>
                        </td>
                        <td width="120">
                          <p>ru</p>
                        </td>
                        <td width="279">
                          <p>Байгаль экологи</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>75</p>
                        </td>
                        <td width="135">
                          <p>Sony Turbo</p>
                        </td>
                        <td width="120">
                          <p>их британи</p>
                        </td>
                        <td width="279">
                          <p>Кино суваг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>76</p>
                        </td>
                        <td width="135">
                          <p>Paramount Channel</p>
                        </td>
                        <td width="120">
                          <p>ану</p>
                        </td>
                        <td width="279">
                          <p>Кино суваг</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>77</p>
                        </td>
                        <td width="135">
                          <p>Sony Sci Fi</p>
                        </td>
                        <td width="120">
                          <p>их британи</p>
                        </td>
                        <td width="279">
                          <p>Кино суваг</p>
                        </td>
                      </tr>
                      {/* <tr>
<td width="96">
<p>78</p>
</td>
<td width="135">
<p>Pobeda</p>
</td>
<td width="120">
<p>ОХУ</p>
</td>
<td width="279">
<p>Кино суваг</p>
</td>
</tr> */}
                      {/* <tr>
<td width="96">
<p>79</p>
</td>
<td width="135">
<p>O!</p>
</td>
<td width="120">
<p>ОХУ</p>
</td>
<td width="279">
<p>Хүүхэд, боловсрол</p>
</td>
</tr> */}
                      <tr>
                        <td width="96">
                          <p>80</p>
                        </td>
                        <td width="135">
                          <p>KHL</p>
                        </td>
                        <td width="120">
                          <p>ru</p>
                        </td>
                        <td width="279">
                          <p>Хүүхэд, боловсрол</p>
                        </td>
                      </tr>
                      {/* <tr>
<td width="96">
<p>81</p>
</td>
<td width="135">
<p>Карусель</p>
</td>
<td width="120">
<p>ОХУ</p>
</td>
<td width="279">
<p>Хүүхэд, боловсрол</p>
</td>
</tr> */}
                      <tr>
                        <td width="96">
                          <p>82</p>
                        </td>
                        <td width="135">
                          <p>Nickelodeon</p>
                        </td>
                        <td width="120">
                          <p>ану</p>
                        </td>
                        <td width="279">
                          <p>Хүүхэд, боловсрол</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>83</p>
                        </td>
                        <td width="135">
                          <p>Match</p>
                        </td>
                        <td width="120">
                          <p>ru</p>
                        </td>
                        <td width="279">
                          <p>Спорт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>84</p>
                        </td>
                        <td width="135">
                          <p>Extreme sport</p>
                        </td>
                        <td width="120">
                          <p>их британи</p>
                        </td>
                        <td width="279">
                          <p>Спорт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>85</p>
                        </td>
                        <td width="135">
                          <p>EURO SPORT</p>
                        </td>
                        <td width="120">
                          <p>герман</p>
                        </td>
                        <td width="279">
                          <p>Спорт</p>
                        </td>
                      </tr>
                      <tr>
                        <td width="96">
                          <p>&nbsp;</p>
                          <p>&nbsp;</p>
                        </td>
                        <td width="135">
                          <p>&nbsp;</p>
                        </td>
                        <td width="120">
                          <p>&nbsp;</p>
                        </td>
                        <td width="279">
                          <p>&nbsp;</p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </td>
              </tr>
            </tbody>
          </table>
          <p>&nbsp;</p>
          <p>Стандарт багц</p>
          <table width="633">
            <tbody>
              <tr>
                <td width="87">
                  <p>
                    <strong>Сувгийн дараалал</strong>
                  </p>
                </td>
                <td width="176">
                  <p>
                    <strong>Телевизийн нэр&nbsp;</strong>
                  </p>
                </td>
                <td width="139">
                  <p>
                    <strong>Харъяа улс</strong>
                  </p>
                </td>
                <td width="232">
                  <p>
                    <strong>Төрөл, жанр</strong>
                  </p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>1</p>
                </td>
                <td width="176">
                  <p>National Промо</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>&nbsp;</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>2</p>
                </td>
                <td>
                  <p>МҮОНТ&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>МҮОНРТ</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>3</p>
                </td>
                <td width="176">
                  <p>Монгол ТВ телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>4</p>
                </td>
                <td width="176">
                  <p>25-р суваг</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>5</p>
                </td>
                <td width="176">
                  <p>ТВ 5 телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>6</p>
                </td>
                <td width="176">
                  <p>UBS Телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>7</p>
                </td>
                <td width="176">
                  <p>Ийгл ньюс телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>8</p>
                </td>
                <td width="176">
                  <p>NTV телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>9</p>
                </td>
                <td width="176">
                  <p>ТВ9 телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>10</p>
                </td>
                <td width="176">
                  <p>Боловсрол суваг&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>11</p>
                </td>
                <td width="176">
                  <p>МNB ММ</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>МҮОНРТ</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>12</p>
                </td>
                <td width="176">
                  <p>Стар телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>13</p>
                </td>
                <td width="176">
                  <p>С1 телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>14</p>
                </td>
                <td width="176">
                  <p>SBN телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>15</p>
                </td>
                <td width="176">
                  <p>&nbsp; Central телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>16</p>
                </td>
                <td width="176">
                  <p>&nbsp;Тэнгэр телевиз&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>17</p>
                </td>
                <td width="176">
                  <p>ТМ телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>18</p>
                </td>
                <td width="176">
                  <p>TV 8 телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>19</p>
                </td>
                <td width="176">
                  <p>ETV телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>20</p>
                </td>
                <td width="176">
                  <p>Сүлд телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>21</p>
                </td>
                <td width="176">
                  <p>Малчин телевиз</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Телевизийн өргөн нэвтрүүлэг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>22</p>
                </td>
                <td width="176">
                  <p>MNB world</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Олон нийтийн&nbsp;</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>23</p>
                </td>
                <td width="176">
                  <p>mnb sport</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Олон нийтийн&nbsp;</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>27</p>
                </td>
                <td width="176">
                  <p>Блүүмберг суваг&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Нийгэм, эдийн засаг</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>28</p>
                </td>
                <td width="176">
                  <p>MNC&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Мэдээ, мэдээлэл</p>
                </td>
              </tr>
              <tr>
                <td width="87">
                  <p>29</p>
                </td>
                <td width="176">
                  <p>Өлзий телевиз&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Нийгэм, эдийн засаг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>30</p>
                </td>
                <td width="176">
                  <p>Нийслэл</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Нийгэм, эдийн засаг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>31</p>
                </td>
                <td width="176">
                  <p>ТВ-4&nbsp;&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Эрүүл мэнд</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>32</p>
                </td>
                <td width="176">
                  <p>Соён гэгээрүүлэгч</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Шинжлэх ухаан, танин мэдэхүй</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>33</p>
                </td>
                <td width="176">
                  <p>Eco Channel</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Байгаль, экологи</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>34</p>
                </td>
                <td width="176">
                  <p>&nbsp;TV-7</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Цэнгээнт</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>35</p>
                </td>
                <td width="176">
                  <p>О TV&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Цэнгээнт</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>36</p>
                </td>
                <td width="176">
                  <p>TV 2&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Цэнгээнт</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>37</p>
                </td>
                <td width="176">
                  <p>Mass TV</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Цэнгээнт</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>38</p>
                </td>
                <td width="176">
                  <p>TV9-2 CINEMA</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Кино суваг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>39</p>
                </td>
                <td width="176">
                  <p>Moviemax&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Кино суваг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>40</p>
                </td>
                <td>
                  <p>DREAMBOX&nbsp;</p>
                </td>
                <td>
                  <p>Монгол</p>
                </td>
                <td>
                  <p>Хүүхэд, боловсрол</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>41</p>
                </td>
                <td width="176">
                  <p>MNBA&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Спорт</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>42</p>
                </td>
                <td width="176">
                  <p>Их засаг ТВ</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Танин мэдэхүй</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>43</p>
                </td>
                <td width="176">
                  <p>Талын Монгол&nbsp;</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Эрүүл мэнд</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>44</p>
                </td>
                <td width="176">
                  <p>&nbsp;ONTV</p>
                </td>
                <td width="139">
                  <p>Монгол</p>
                </td>
                <td width="232">
                  <p>Нийгэм эдийн засаг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>45</p>
                </td>
                <td>
                  <p>Zvezda</p>
                </td>
                <td width="139">
                  <p>Орос</p>
                </td>
                <td width="232">
                  <p>Нийгэм, эдийн засаг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>46</p>
                </td>
                <td>
                  <p>DW</p>
                </td>
                <td width="139">
                  <p>Герман</p>
                </td>
                <td width="232">
                  <p>Нийгэм, эдийн засаг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>47</p>
                </td>
                <td>
                  <p>France-24</p>
                </td>
                <td width="139">
                  <p>Франц</p>
                </td>
                <td width="232">
                  <p>Нийгэм, эдийн засаг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>48</p>
                </td>
                <td>
                  <p>NHK</p>
                </td>
                <td width="139">
                  <p>Япон</p>
                </td>
                <td width="232">
                  <p>Нийгэм, эдийн засаг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>49</p>
                </td>
                <td width="176">
                  <p>&nbsp;Travel+</p>
                </td>
                <td width="139">
                  <p>сингапур</p>
                </td>
                <td width="232">
                  <p>Байгаль экологи</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>50</p>
                </td>
                <td width="176">
                  <p>&nbsp;Өвөрмонгол</p>
                </td>
                <td width="139">
                  <p>Хятад</p>
                </td>
                <td width="232">
                  <p>Нийгэм, эдийн засаг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>51</p>
                </td>
                <td>
                  <p>NTV rus</p>
                </td>
                <td width="139">
                  <p>Орос</p>
                </td>
                <td width="232">
                  <p>Цэнгээнт</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>52</p>
                </td>
                <td>
                  <p>&nbsp;DocuBox</p>
                </td>
                <td width="139">
                  <p>Их Британи</p>
                </td>
                <td width="232">
                  <p>Цэнгээнт</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>53</p>
                </td>
                <td>
                  <p>TV-1000</p>
                </td>
                <td width="139">
                  <p>Их Британи</p>
                </td>
                <td width="232">
                  <p>Кино суваг</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>54</p>
                </td>
                <td>
                  <p>Disney</p>
                </td>
                <td width="139">
                  <p>АНУ</p>
                </td>
                <td width="232">
                  <p>Хүүхэд, боловсрол</p>
                </td>
              </tr>
              <tr>
                <td>
                  <p>55</p>
                </td>
                <td>
                  <p>2x2&nbsp;&nbsp;&nbsp;&nbsp;</p>
                </td>
                <td width="139">
                  <p>Орос</p>
                </td>
                <td width="232">
                  <p>Хүүхэд, боловсрол</p>
                </td>
              </tr>
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}

export default Help2;
