...
import { getEguneAnswer } from "@/lib/chatbot/egune";
import {
  buildChatbotInstructions,
  buildEguneInstructions,
} from "@/lib/chatbot/prompt";
import { getCrawledSiteContext } from "@/lib/chatbot/site-crawler";
import {
  detectChatAction,
  detectPromisedChatAction,
...
import {
  getOutOfScopeAnswer,
  isTelecomSupportRequest,
} from "@/lib/chatbot/scope";
...
type AiProvider = "gemini" | "openai" | "egune";
...
function getAiProviders(): AiProvider[] {
  ...
  // Explicit Egune selection does not silently forward conversations elsewhere.
  if (configuredProvider === "egune") {
    return process.env.EGUNE_API_KEY?.trim() ? ["egune"] : [];
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!isTelecomSupportRequest(parsed.data.messages, parsed.data.pathname)) {
    ...
    return json({
      answer: getOutOfScopeAnswer(parsed.data.locale),
      requestId,
      source: "scope-guard",
      action: null,
    });
  ...
  if (providers.length === 0) {
    ...
    return json(
      {
        error:
          "Туслах түр боломжгүй байна. Дараа дахин оролдоно уу.",
      },
      503
    );
  }
  ...
  try {
    ...
    for (const provider of providers) {
      try {
        answer =
          provider === "egune"
            ? await getEguneAnswer(parsed.data.messages, eguneInstructions)
            : provider === "gemini"
    ...
    const responseAction =
      action ?? detectPromisedChatAction(answer, parsed.data.messages);
    ...
    return json({
      answer: sanitizeActionAnswer(answer, responseAction, parsed.data.locale),
      requestId,
      source: knowledgeContext
        ? `${providerUsed}-grounded`
        : providerUsed,
