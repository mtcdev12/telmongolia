...
export function detectPromisedChatAction(
  answer: string,
  messages: ConversationMessage[]
): ChatAction | null {
  const normalizedAnswer = answer.toLocaleLowerCase("mn");
  const promisesActionPanel =
    /(доорх|дараах).{0,48}(форм|хэсэг|сонголт|асуулт)|form below|below.{0,48}(form|questions|options)/iu.test(
      normalizedAnswer
    );

  if (!promisesActionPanel) return null;

  const recentContext = [...messages.slice(-6).map((message) => message.content), answer]
    .join("\n")
    .toLocaleLowerCase("mn");

  if (/(багц|тариф|үйлчилгээ|plan|package|service)/iu.test(recentContext)) {
    return "package-adviser";
  }

  if (
    /(дугаар|суурин утас|phone number|telephone number|new number)/iu.test(
      recentContext
    )
  ) {
    return "number-order";
  ...
  if (/(засвар|гэмтэл|repair|ticket)/iu.test(recentContext)) {
    return "ticket-create";
  }

  if (/(төлбөр|нэхэмжлэл|bill|invoice)/iu.test(recentContext)) {
    return "bill-explain";
  }

  if (/(салбар|цаг авах|appointment|branch)/iu.test(recentContext)) {
    return "appointment-book";
  }

  if (/(ажилтан|оператор|human agent|employee|operator)/iu.test(recentContext)) {
    return "human-handoff";
  }

  if (/(тасалдал|саатал|сүлжээ|outage|service down|network issue)/iu.test(recentContext)) {
    return "outage-check";
  }
