...
const CHAT_CONTEXT_WAIT_MS = 2_000;
const MAX_PAGES = 60;
...
function extractLinks(html: string, origin: string) {
  ...
  for (const match of Array.from(html.matchAll(
    /<a\b[^>]*\bhref\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/gi
  ))) {
...
export async function getCrawledSiteContext(
  ...
  try {
    // The initial crawl can span dozens of pages. Keep building the cache, but
    // answer the current request with verified structured facts if it is slow.
    const index = await Promise.race([
      getSiteIndex(origin),
      new Promise<null>((resolve) => { timer = setTimeout(() => resolve(null), CHAT_CONTEXT_WAIT_MS); }),
    ]);
    if (!index) return "";
