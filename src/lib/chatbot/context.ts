...
const serviceAliases: Array<{ service: string; aliases: string[] }> = [
  ...
  {
    service: "Суурин утас",
    aliases: ["суурин утас", "suurin utas", "дан багц", "ярианы багц"],
  },
  {
...
function findAudience(text: string): Audience | null {
  if (
    /\b(байгууллага|компани|бизнес|албан газар|аж ахуйн|baiguullaga|company|business)\b/.test(
      text
    )
  ) {
    return "Байгууллага";
  ...
  if (
    /\b(өрх|өрхийн|гэр|хувь хүн|orh|orkh|orhin|orkhin|origin|ger|huvi hun)\b/.test(
      text
    )
  ) {
...
function hasProductIntent(text: string) {
  return /(үйлчилгээ|багц|үнэ|тариф|хурд|нөхцөл|дагалдах|сонголт|зөвлө|хэд вэ|ямар вэ|хамгийн|интернэт|интернет|телевиз|утас|uilchilg|uiclhilg|uichilg|bagts|une|tarif|hurd|nohtsol|songolt|zuvl|sonirho|internet|televiz|utas)/.test(
    text
  );
}
